...
# outputs:
#   - Slope GeoTIFF for Alberta, either aligned to the ABMI
#     1 km reference grid or at BASE_SCALE_M resolution,
#     selected by EXPORT_TARGET (exported to Google Drive)
# notes:
#   Slope in degrees from the FABDEM bare-earth DEM (30 m,
#   forests and buildings removed). The collection is mosaicked
#   and pinned to a BASE_SCALE_M metric projection, slope is
#   computed, then aggregated to the ABMI 1 km reference grid.
#
#   Data citation:
...
# 1.1 User parameters ----
BAND_NAME = "slope"
TASK_PREFIX = "FABDEM_Slope_Alberta"
...
layer = ee.Terrain.slope(elevation).rename(BAND_NAME)
